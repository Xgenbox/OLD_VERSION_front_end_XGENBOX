...
function CleaningService() {
  ...
  const cleaningService = useSelector(state=>state.cleaningService?.requestsSent)
  const history = useHistory();
  const dt = useRef(null);
  useEffect(() => {
    dispatch(findSingleCleaningService())
...
const [globalFilterValue, setGlobalFilterValue] = useState('');
  ...
  console.log("listOfBins", listOfBins)
  ...
  const deleteBin = (id)=> {
    ...
    dispatch(DeleteRequestCleaningService({requestId:id}))
    // if(isSuccess){
              ...
              <CardHeader className="border-0">
                <Row>
                  ...
                  // lg="6"
                    md="10"
                  >
                <h3 className="mb-0">List Of Cleaning service request created by you </h3>
                  ...
                  >
                     <Link
                          to={`/citizen/createCleaningservice`}
                          >
                            <Button
    ...
    <div className="spinner-border text-light" role="status">
      <span className="visually-hidden"></span>
            ...
            <div className="card">
              ...
              <DataTable paginator rows={5} rowsPerPageOptions={[5, 10, 25]} ref={dt} value={cleaningService} header={header} selection={selectedProduct}
              selectionMode={true}
